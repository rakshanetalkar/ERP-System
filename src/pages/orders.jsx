...
const OrdersManagement = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <h2 className="text-2xl font-bold mb-4 text-center bg-clip-text text-transparent bg-cyan-500">Orders Management</h2>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id} className="hover:bg-gray-100">
                ...
                <td className="py-2 px-4 border text-center">{order.customerName}</td>
                <td className="py-2 px-4 border text-center">{order.orderDate}</td>
                <td className="py-2 px-4 border text-center">{order.status}</td>
                <td className="py-2 px-4 border flex md:flex-row justify-center">
                  <button
      ...
      {selectedOrder && open && (
        <div className="mt-4 p-4 border rounded-md shadow-md text-center">
          <h3 className="text-2xl font-bold mb-2">Order Details</h3>
          <p className="text-xl">Order ID: {selectedOrder.orderID}</p>
          <p className="text-xl">Customer: {selectedOrder.customerName}</p>
          <p className="text-xl">Order Date: {selectedOrder.orderDate}</p>
          <p className="text-xl">Status: {selectedOrder.status}</p>
        </div>
      )}
    </div>
  );
